/* $Id: assemble.c,v 1.12 2023/11/06 09:36:24 leavens Exp $ */
#include <stdio.h>
#include <string.h>
#include "ast.h"
#include "utilities.h"
...
void assembleProgram(BOFFILE bf, program_t prog)
{
	...
	* ast_list_length(prog.textSection.instrs.instrs);
    ...
    bh.ints_length = BYTES_PER_WORD
	* ast_list_length(prog.dataSection.staticIntDecls.int_decls);
    bh.floats_length = BYTES_PER_WORD
	* ast_list_length(prog.dataSection.staticFloatDecls.float_decls);
...
{
    assembleStaticIntDecls(bf, ds.staticIntDecls);
    assembleStaticFloatDecls(bf, ds.staticFloatDecls);
}

// Assemble the code for the given AST, with output going to bf
void assembleStaticIntDecls(BOFFILE bf, static_int_decls_t sids)
{
    static_int_decl_t *dcl = sids.int_decls;
    while (dcl != NULL) {
	assembleStaticIntDecl(bf, *dcl);
	dcl = dcl->next;
    }
}

// Assemble the code for the given AST, with output going to bf
void assembleStaticFloatDecls(BOFFILE bf, static_float_decls_t sfds)
{
    static_float_decl_t *dcl = sfds.float_decls;
    while (dcl != NULL) {
	assembleStaticFloatDecl(bf, *dcl);
	dcl = dcl->next;
    }
}

// Assemble the code for the given AST, with output going to bf
void assembleStaticIntDecl(BOFFILE bf, static_int_decl_t dcl)
{
    bof_write_int(bf, dcl.value);
}

// Assemble the code for the given AST, with output going to bf
void assembleStaticFloatDecl(BOFFILE bf, static_float_decl_t dcl)
{
    bof_write_float(bf, dcl.value);
}
