/* $Id: bof.c,v 1.11 2023/11/13 00:25:37 leavens Exp $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "bof.h"
...
int_type bof_read_int(BOFFILE bf)
{
    word_pun_t b;
    size_t bytes_read = bof_read_bytes(bf, BYTES_PER_WORD, b.buf);
    if (bytes_read == 0) {
	bail_with_error("Cannot read a int from %s (got %d bytes: 0x%x), the file is at EOF: %d",
			bf.filename, bytes_read, b.ival, feof(bf.fileptr));
    }
    return b.ival;
}
...
float_type bof_read_float(BOFFILE bf)
{
    word_pun_t b;
    size_t bytes_read = bof_read_bytes(bf, BYTES_PER_WORD, b.buf);
    if (bytes_read == 0) {
	bail_with_error("Cannot read a float from %s (got %d bytes: 0x%x), the file is at EOF: %d",
			bf.filename, bytes_read, b.fval, feof(bf.fileptr));
    }
    return b.fval;
}
...
BOFHeader bof_read_header(BOFFILE bf) {
    ...
    if (strncmp(ret.magic, "FBF", MAGIC_BUFFER_SIZE) != 0) {
...
// Exit the program with an error if this fails.
extern void bof_write_int(BOFFILE bf, int_type ival)
{
    word_pun_t wp;
    wp.ival = ival;
    bof_write_bytes(bf, BYTES_PER_WORD, wp.buf);
}


// Requires: bf is open for writing in binary
//           and the size of buf is at least BYTES_PER_WORD bytes
// Write the given float (fval) into bf
// Exit the program with an error if this fails.
extern void bof_write_float(BOFFILE bf, float_type fval)
{
    word_pun_t wp;
    wp.fval = fval;
    bof_write_bytes(bf, BYTES_PER_WORD, wp.buf);
...
void bof_write_header(BOFFILE bf, const BOFHeader hdr) {
    ...
    if (wr != 1) {
	bail_with_error("Cannot write header to %s", bf.filename);
    }
    /*
