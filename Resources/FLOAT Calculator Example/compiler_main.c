/* $Id: compiler_main.c,v 1.12 2023/11/12 02:20:22 leavens Exp $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "lexer.h"
...
static void usage(const char *cmdname)
{
    fprintf(stderr, "Usage: %s %s\n       %s %s\n       %s %s\n",
	    cmdname, "-l codeFilename.flt",
	    cmdname, "-u codeFilename.flt",
	    cmdname, " codeFilename.flt"
	    );
    exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    if (lastdot == NULL || strcmp(lastdot, ".flt") != 0) {
	bail_with_error("filename argument must end in .flt, not %s",
			filename);
    }
    ...
    program_t progast = parseProgram(filename);
    ...
    // check types and record types in the AST
    progast.stmt = type_check_stmt(progast.stmt);
